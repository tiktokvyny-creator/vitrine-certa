...
const JWT_ADMIN_AAL1 = "jwt-admin-aal1-token-xyz";
...
function makeRepo(products: FullProduct[] = []) {
  ...
  const repo: Repo = {
    getAuthIdentity: async (jwt) => jwt === JWT_ADMIN ? { userId: ADMIN, aal: "aal2" }
      : jwt === JWT_ADMIN_AAL1 ? { userId: ADMIN, aal: "aal1" }
      : jwt === JWT_USER ? { userId: USER, aal: "aal2" } : null,
...
Deno.test("admin em AAL1: 403 mfa_required e nada executado", async () => {
  const m = makeRepo(); const { deps, fetchCalls } = makeDeps(m.repo);
  const r = await call(deps, { action: "start_sync" }, { jwt: JWT_ADMIN_AAL1 });
  assertEquals(r.status, 403); assertEquals(r.json.code, "mfa_required");
  assertEquals(fetchCalls.length, 0); assertEquals(m.runs.length, 0);
});
