...
export async function handle(req: Request, deps: Deps): Promise<Response> {
  ...
  // Autenticação: sessão válida + presença na tabela admins.
  const auth = req.headers.get("authorization") ?? "";
  const jwt = auth.startsWith("Bearer ") ? auth.slice(7).trim() : "";
  if (!jwt) return fail(401, "unauthenticated", "Sessão ausente. Entre novamente.", cors);
  let userId: string | null = null;
  try { userId = await repo.getUserId(jwt); } catch { userId = null; }
  if (!userId) return fail(401, "unauthenticated", "Sessão inválida ou expirada. Entre novamente.", cors);
