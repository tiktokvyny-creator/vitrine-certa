...
const repo: Repo = {
  async getAuthIdentity(jwt) {
    const [{ data: userData, error: userError }, { data: claimsData, error: claimsError }] = await Promise.all([
      anon.auth.getUser(jwt),
      anon.auth.getClaims(jwt),
    ]);
    const claims = claimsData?.claims as { sub?: string; aal?: string } | undefined;
    if (userError || claimsError || !userData?.user || !claims || claims.sub !== userData.user.id) return null;
    return { userId: userData.user.id, aal: typeof claims.aal === "string" ? claims.aal : null };
  },
